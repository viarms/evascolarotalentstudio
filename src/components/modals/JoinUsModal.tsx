...
interface FieldErrors {
  parentName?: string; childName?: string; childAge?: string;
  classes?: string; studio?: string; whatsapp?: string; email?: string;
}
...
export default function JoinUsModal() {
  ...
  useEffect(() => {
    window.addEventListener("open-join-us-modal", open);
    return () => window.removeEventListener("open-join-us-modal", open);
  }, [open]);
  ...
  const handleClassToggle = (c: string) => {
    const next = fields.classes.includes(c)
      ? fields.classes.filter(x => x !== c)
      : [...fields.classes, c];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setTouched({ parentName: true, childName: true, childAge: true, classes: true, studio: true, whatsapp: true, email: true });
    const errs = validate(fields);
    setErrors(errs);
    if (Object.keys(errs).length > 0) return;
    setStatus("submitting");
    try {
      const res  = await fetch("/api/join-us", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(fields) });
