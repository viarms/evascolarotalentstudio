...
//
// Replicates WPForms 2207 / Elementor popup 840 — Book Free Trial form.
...
export default function BookTrialModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTouched({
      ...
      classInterest: true, studio: true, whatsapp: true, email: true,
    });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          type="button" onClick={closeModal} aria-label="Close book trial form"
          ...
          {status === "success" ? (
            <div className="flex flex-col items-center gap-4 py-8 text-center">
              ...
              <p className="text-sm text-gray-500">
                We&apos;ll confirm your trial soon. Or reach us directly on{" "}
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              {/* Class Interest + Preferred Studio side by side */}
              <div className="flex gap-3 mb-4">
                <div className="flex-1">
                  <label htmlFor={`${uid}-classInterest`} className={cls.label}>
                    Class Interest <span aria-hidden="true" className="text-red-600">*</span>
                  </label>
                  <select
                    id={`${uid}-classInterest`} name="classInterest"
                    value={fields.classInterest} onChange={handleChange} onBlur={handleBlur}
                    disabled={status === "submitting"}
                    aria-invalid={!!errors.classInterest}
                    aria-describedby={errors.classInterest ? `${uid}-class-err` : undefined}
                    className={`${errors.classInterest ? cls.error : cls.ok} cursor-pointer`}
                  >
                    <option value="">— Select class —</option>
                    {CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  {errors.classInterest && (
                    <p id={`${uid}-class-err`} role="alert" className="mt-1 text-xs text-red-600">
                      {errors.classInterest}
                    </p>
                  )}
                </div>
              ...
              <div className={cls.field}>
                ...
                <select
                  id={`${uid}-preferredDay`} name="preferredDay"
